...
def get_input_from_the_user():
    while True:
        ...
        #            NameOfChat = input("Type the name of a new chat?:   ")
            ...
            fb.post('Message',
                    {"Message": f'Screenshot ({x}) was uploaded'})  # create message that the screenshot was uploaded

        elif message == '/DS':
            x = 'monitor-1.png'
            storage.child(f'FB/{x}').download(f"{x}")  # download screenshot from the firebase storage
            fb.post('Message', {
                "Message": f'Screenshot ({x}) was downloaded'})  # create message that the screenshot was downloaded

        elif message == "/clear":
            fb.delete('Message', '')

        elif message == "/newChat":
            newChatName = input("Enter new chat name:   ")
            fb.put(f'Message/{newChatName}/', 'sth')
        ...
        # if the message starts with "/reply"
            ...
            if len(msg) > 3:
                ...
                fb.put(f'Message/{message_id}/', 'Message',
        ...
        # if the message starts with "/edit"
            ...
            if len(msg) > 2:
                ...
                fb.put(f'Message/{message_id}/', 'Message',
                       str(datetime.now().time())[:8] + username + "EDITED " + edited_message)
